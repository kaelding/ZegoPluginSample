...
extension ConversationListVC: UITableViewDelegate {
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let messageListVC = MessagesListVC(conversationID: model.id, type: model.type, conversationName: model.name)
        self.navigationController?.pushViewController(messageListVC, animated: true)
        
        // clear unread messages
        viewModel.clearConversationUnreadMessageCount(model.id, type: model.type)
    }
