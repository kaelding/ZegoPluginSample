    ...
    func loadConversationInfo() {
        if conversationType == .peer {
            viewModel.queryOtherUserInfo { [weak self] error in
                if error.code == .success {
                    self?.navigationItem.title = self?.viewModel.otherUser?.name
                    self?.tableView.reloadData()
                }
            }
...
extension MessagesListVC: ImageMessageCellDelegate,
    ...
    func imageMessageCell(_ cell: ImageMessageCell, didClickImageWith messageVM: ImageMessageViewModel) {
        ...
        let index = viewModels.firstIndex(where: { $0.message === messageVM }) ?? 0
...
extension MessagesListVC {
    func scrollToBottom(_ animated: Bool) {
        if viewModel.messageViewModels.count > 0 {
            let index = IndexPath(row: viewModel.messageViewModels.count-1, section: 0)
